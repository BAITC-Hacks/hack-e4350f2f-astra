...
Городской симулятор на HTML, CSS и обычном JavaScript. Установка зависимостей и сервер не нужны.

Откройте `index.html` двойным кликом в браузере. Файлы `index.html`, `styles.css` и `app.js` должны находиться в одной папке.

- `index.html` — структура страницы.
- `styles.css` — оформление и адаптация под телефон.
- `app.js` — 14 мер, 5 районов, фильтры, бюджет и симуляция.

Выберите до пяти мер, укажите районы и нажмите «Запустить симуляцию и анализ AI». Превышение бюджета допускается для анализа и выделяется красным. Изменение выбранных мер или их районов сбрасывает прогноз.

AI-вердикт формируется локально по шаблону. Эффекты мер демонстрационные, Score меняется с 52.56 на фиксированные 56.50 (+3.94).

`App.jsx` и папка `dashboard` относятся к предыдущей React-версии и не нужны для запуска HTML-версии.
