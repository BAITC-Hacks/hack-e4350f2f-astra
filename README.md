...
Проект по задаче хакатона «Аким на 5 часов».
...
Снимки восстанавливаются после перезагрузки страницы. «Скачать JSON» сохраняет оба результата,
...
Интерфейс разделён на пять вкладок: «Решения», «Районы и результат», «AI-советник»,
«Сравнение» и «Как играть». Панель навигации остаётся сверху при прокрутке,
под ней — подсказка для текущего шага и кнопка перехода к следующему разделу.
Между кнопками навигации можно переключаться стрелками; Home/End открывают первый/последний раздел.
Бюджет, счётчик решений и кнопка расчёта всегда доступны в нижней панели.
После расчёта открываются результаты. В разделе районов выберите нужный район,
чтобы увидеть его десять показателей. AI-текст и PDF находятся во вкладке советника,
сохранение A/B и JSON — во вкладке сравнения, удаление сохранённых данных — в «Как играть».
Нажатие на выбранную инициативу в коротком списке над карточками убирает её из пакета.
...
Выбранные меры, назначенные районы и активный фильтр автоматически сохраняются
в браузере и восстанавливаются после обновления страницы. Восстановленный выбор
проверяется по актуальному каталогу и правилам; несовместимые или устаревшие меры
убираются с уведомлением. «Сбросить» очищает сохранённый набор.
Для восстановления используйте тот же браузер и адрес, включая порт.
Если браузер запрещает локальное хранилище, симулятор работает без сохранения.
Результат расчёта, AI-ответ и сценарии A/B также сохраняются и восстанавливаются.
Обновление страницы не запускает новый платный AI-запрос. Если страница закрылась во время
анализа, сохранённый расчёт восстанавливается с кнопкой повторного запроса AI.
Сохранение привязано к адресу сайта, включая порт: localhost и 127.0.0.1 имеют разные хранилища.
В другом браузере или на другом устройстве данные не появятся. При изменении каталога,
показателей, правил или версии формата старые расчёты и анализы сбрасываются; выбор
проверяется по новым правилам. Повреждённое сохранение не мешает работе приложения.
«Удалить сохранённые данные» удаляет только сохранение симулятора и очищает текущий выбор,
результат, анализ и A/B. «Сбросить» очищает только текущий сценарий, а «Очистить сравнение» — A/B.
При запрете localStorage или нехватке места сайт показывает предупреждение и продолжает работать.
...
## Отчёт для защиты (PDF)

После расчёта и завершения попытки AI-анализа нажмите «Скачать отчёт (PDF)»
под вердиктом. В окне печати выберите «Сохранить как PDF» (на macOS — меню PDF).
Это сохранение через браузер, а не автоматическое скачивание файла.

В отчёт входят текущий рассчитанный набор мер, бюджет, Score до/после, синергии,
индексы и все показатели районов, а также полученный AI-анализ. Если AI недоступен,
отчёт сохраняется с явной пометкой об отсутствии анализа. После изменения выбора
нужно заново выполнить расчёт. Сценарии A/B экспортируются отдельно кнопкой «Скачать JSON».
Печать не отправляет новый запрос к AI и не расходует квоту.
...
Проверка интерфейса и PDF в установленном Google Chrome (macOS/Linux):

```bash
python browser_smoke.py
```

Windows использует установленный Microsoft Edge:
...
бюджет, несовместимости, смену Score, повтор AI, экспорт PDF и мобильную ширину, затем останавливает сервер.
Пробный PDF сохраняется в `.venv/report-smoke.pdf`; в Git он не попадает.
