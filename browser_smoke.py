            ...
            with sync_playwright() as p:
                ...
                def click(selector):
                    if selector.startswith('[data-view='):
                        ...
                        return
                    if selector in ('#save-A', '#save-B', '#clear-scenarios', '#export-scenarios'):
                        view = 'compare'
                    elif selector in ('#export-report', '#retry-analysis'):
                        view = 'advisor'
                    elif selector == '#delete-saved':
                        view = 'help'
                    elif selector == '#simulate':
                        view = None
                    else:
                        view = 'decisions'
                    if view:
                ...
                expect(page).to_have_title("QalaAI: Цифровой советник акима")
                expect(page.locator("#hint-title")).to_contain_text("Соберите пакет")
                page.locator("#hint-next").click()
                expect(page.locator('[data-panel="results"]')).to_be_visible()
                expect(page.locator("#hint-text")).to_contain_text("исходное состояние")
                page.locator('[data-view="results"]').focus()
                page.keyboard.press("ArrowRight")
                expect(page.locator('[data-panel="advisor"]')).to_be_visible()
                ...
                expect(report).to_contain_text("QalaAI: Цифровой советник акима")
                ...
                expect(page.locator('[data-panel="results"]')).to_be_visible()
                expect(page.locator('[data-panel="decisions"]')).not_to_be_visible()
                page.locator('[data-pick-district]').first.click()
                expect(page.locator('.district:visible')).to_have_count(1)
